    ...
    <div className="fade-in">
      ...
      <Modal open={!!dupTarget} title="Duplicate Party Type" onClose={() => setDupTarget(null)}
        ...
        <div className="cmp-form-grid">
          <div className="cmp-field--full">
            <label className="cmp-label">Name <span className="cmp-required">*</span></label>
            <Input value={newName} placeholder="e.g., Plaintiff" onChange={e => setNewName(e.target.value)} />
          </div>
          <div className="cmp-field--full">
            <label className="cmp-label">Short Code <span className="cmp-required">*</span></label>
      ...
      <div className="cmp-table-card">
        <table className="cmp-table">
          <thead>
            <tr>
              {bulkMode && <th className="cmp-th--w40"><input type="checkbox" checked={selected.size === filtered.length && filtered.length > 0} onChange={toggleAll} /></th>}
          ...
          <tbody>
            {paged.length === 0 ? (
              <tr><td className="cmp-empty" colSpan={bulkMode ? 7 : 6}>No party types found.</td></tr>
            ) : paged.map((item, idx) => (
              <tr key={item.id}
                draggable={!search}
              ...
              >
                {bulkMode && <td className="cmp-check-cell"><input type="checkbox" checked={selected.has(item.id)} onChange={() => toggleBulkDel(item.id)} /></td>}
      ...
      <div className="cmp-mobile-only">
        ...
        <div className="cmp-mobile-section-header">
          <span className="cmp-mobile-section-title">All Party Types</span>
        ...
        <div className="cmp-mobile-list">
          {paged.length === 0 ? (
            <div className="cmp-empty">No party types found.</div>
          ) : paged.map(item => (
