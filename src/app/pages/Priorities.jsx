...
export default function Priorities() {
  ...
  const load = async () => {
    setLoading(true);
    const res = await priorityLogic.normalizeOrder().catch(() => []);
  ...
  const doDelete = async () => {
    ...
    setConfirmState({
      ...
      message: `Delete priority "${item?.name}"?`,
      variant: 'danger',
      confirmLabel: 'Delete',
      onConfirm: async () => {
  ...
  const doBulkDelete = async () => {
    ...
    setConfirmState({
      ...
      message: `Delete ${count} priority(s)?`,
      variant: 'danger',
      ...
      onConfirm: async () => {
        ...
        for (let idx = 0; idx < ids.length; idx++) {
          ...
          await priorityLogic.remove(ids[idx]);
        }
        setBusy(false);
        setProgress(null);
        setBulkDelSelected(new Set());
  ...
  const confirmDeleteItem = (item) => {
    setConfirmState({
      title: 'Delete Priority',
      message: `Delete priority "${item?.name}"? This action cannot be undone.`,
      variant: 'danger',
      confirmLabel: 'Delete',
      onConfirm: async () => {
        setConfirmState(null);
        ...
        setBusy(false);
        if (res.ok || !res.error) { toast.push('Priority deleted.', 'success'); load(); }
        else toast.push(res.error, 'error');
      },
      onCancel: () => setConfirmState(null),
    });
...
<ColorPicker value={editColor} onChange={setEditColor} />
      ...
      <div className="cmp-table-card">
        <table className="cmp-table">
          ...
          <tbody>
            {paged.length === 0 ? (
              <tr><td className="cmp-empty" colSpan={bulkMode ? 7 : 6}>No priorities found.</td></tr>
            ) : paged.map((item, idx) => (
      ...
      <div className="cmp-mobile-only">
        <div className="cmp-mobile-stats">
          <div className="cmp-mobile-stat-card">
            <div className="cmp-mobile-stat-row1">
              <div className="cmp-mobile-stat-icon cmp-mobile-stat-icon--total"><Icon name="flag" size={16} /></div>
        ...
        <div className="cmp-mobile-section-header">
          <span className="cmp-mobile-section-title">All Priorities</span>
        ...
        <div className="cmp-mobile-list">
          {paged.length === 0 ? (
            <div className="cmp-empty">No priorities found.</div>
          ) : paged.map(item => (
            <div key={item.id} className="cmp-mobile-card">
              <div className="cmp-mobile-card-row1">
                ...
                <span className="cmp-mobile-avatar cmp-mobile-avatar--import" style={{ '--avatar-bg': item.color || '#EEF1FB' }}><Icon name="flag" size={18} /></span>
