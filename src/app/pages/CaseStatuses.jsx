    ...
    if (Array.isArray(res)) {
      ...
      if (allZero) {
        ...
        for (const item of data) {
          await caseStatusLogic.update(item.id, { display_order: item.display_order }).catch(() => {});
        }
      }
      setItems(data);
    }
  ...
  return (
    <div className="fade-in">
      <div className="cmp-hero">
        <div className="cmp-hero-icon"><Icon name="toggle" size={30} /></div>
      ...
      <div className="cmp-search-row">
        <div className="cmp-search">
          ...
          <input value={search} placeholder="Search case statuses…" onChange={e => { setSearch(e.target.value); setPage(1); }} />
        </div>
        <div className="cmp-stat">
          <div className="cmp-stat-icon"><Icon name="layers" size={20} /></div>
          <div>
            <div className="cmp-stat-label">Total Case Statuses</div>
      ...
      <div className="cmp-table-card">
        <table className="cmp-table">
          ...
          <tbody>
            {paged.length === 0 ? (
              <tr><td className="cmp-empty" colSpan={5}>No case statuses found.</td></tr>
            ) : paged.map((item, idx) => (
        ...
        <div className="cmp-table-footer">
          <div>Showing {(safePage - 1) * PER_PAGE + 1} to {Math.min(safePage * PER_PAGE, filtered.length)} of {filtered.length} case statuses</div>
          {totalPages > 1 && (
