...
export default function Jurisdictions() {
  ...
  const doAdd = async () => {
    ...
    setBusy(true);
    const res = await jurisdictionLogic.create({ name: newName, short_code: newCode, status: newStatus, description: newDesc, color: newColor });
    setBusy(false);
    if (res.ok) { setNewName(''); setNewCode(''); setNewStatus('Active'); setNewDesc(''); setNewColor('#6b7280'); setDupTarget(null); toast.push('Jurisdiction added.', 'success'); load(); }
  ...
  const doEdit = async () => {
    ...
    const res = await jurisdictionLogic.update(editId, { name: editName, short_code: editCode, description: item?.description, display_order: item?.display_order, status: editStatus, color: newColor });
  ...
  const doDelete = async () => {
    ...
    setConfirmState({
      ...
      message: `Delete jurisdiction "${item?.name}"?`,
      variant: 'danger',
      confirmLabel: 'Delete',
      onConfirm: async () => {
  ...
  const doBulkDelete = async () => {
    ...
    setConfirmState({
      title: 'Delete Jurisdictions',
      message: `Delete ${count} jurisdiction(s)?`,
      variant: 'danger',
      ...
      onConfirm: async () => {
        ...
        for (let idx = 0; idx < ids.length; idx++) {
          ...
          await jurisdictionLogic.remove(ids[idx]);
        }
        setBusy(false);
        setProgress(null);
        setBulkDelSelected(new Set());
  ...
  const confirmDeleteItem = (item) => {
    setConfirmState({
      title: 'Delete Jurisdiction',
      message: `Delete jurisdiction "${item?.name}"? This action cannot be undone.`,
      variant: 'danger',
      confirmLabel: 'Delete',
      onConfirm: async () => {
        setConfirmState(null);
        ...
        setBusy(false);
        if (res.ok || !res.error) { toast.push('Jurisdiction deleted.', 'success'); load(); }
        else toast.push(res.error, 'error');
      },
      onCancel: () => setConfirmState(null),
    });
            ...
            {activeAction === 'add' && subMode === 'single' && (
              <div className="cmp-form-grid">
                ...
                <div className="cmp-field">
                  ...
                  <Input value={newCode} placeholder="JURT-CIVIL" onChange={e => setNewCode(e.target.value.toUpperCase())} onKeyDown={e => e.key === 'Enter' && doAdd()} />
              ...
              <div className="cmp-form-grid">
                ...
                {editId && (
                  <>
                    ...
                    <div className="cmp-field">
                      <label className="cmp-label">Short Code <span className="cmp-required">*</span></label>
                      <Input value={editCode} placeholder="JURT-CIVIL" onChange={e => setEditCode(e.target.value.toUpperCase())} />
                      <span className="cmp-hint">Example: JURT-CIVIL</span>
                    </div>
                    <div className="cmp-field">
                      <label className="cmp-label">Status</label>
