...
export default function CaseHistory({ caseId, onChanged }) {
  ...
  const importCauseList = async () => {
    const res = await caseHistoryLogic.importFromCauseList(caseId, user);
  ...
  return (
    ...
    >
      ...
      {filtered.length === 0 ? <EmptyState icon="history" title="No history yet." hint="Import from the cause list or add an entry." /> : (
              ...
              return (
                <div className="timeline-item" key={h.id}>
                  <div className="timeline-item__date">
                    ...
                    {h.source === 'cause-list' && <Badge tone="grey">cause list</Badge>}
                    <span style={{ flex: 1 }} />
                  </div>
                  <div className="timeline-item__event" style={{ whiteSpace: 'pre-wrap' }}>{shown}</div>
                  <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
                    {long && <button className="linkbtn" onClick={() => setExpanded((e) => ({ ...e, [h.id]: !isOpen }))}>{isOpen ? 'Collapse' : 'Read full text'}</button>}
                    <PermissionGate perm="casevault.edit"><button className="linkbtn linkbtn--danger" onClick={() => remove(h)}>Delete</button></PermissionGate>
                  </div>
                </div>
              );
            })}
